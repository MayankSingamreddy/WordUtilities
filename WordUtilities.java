...
public class WordUtilities {
	...
	public int binarySearchIterative(List<String> listOfWords, String target) {
		...
		//the initial check before every binary search 
		while(low<= high){
			int mid = (high+low)/2; //creates the midpoint to search from
			int compare = target.compareTo(listOfWords.get(mid)); //compares from middle
			
			if (compare == 0) //if equal then return mid
				return mid;
			else if (compare <0) high = mid -1; //another case for if its lower
			else low = mid +1; //default case if it is higher
		}
